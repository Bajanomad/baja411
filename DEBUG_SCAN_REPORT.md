...
> ⚠️ **Historical Report (Point-in-Time Scan):** This document reflects a static scan performed at a prior point in time and may include stale recommendations. The current source of truth for architecture and implementation guidance is `REPO_MAP.md`.
> 
> Current confirmed direction:
> - Native forecast UI remains in `app/weather/page.tsx` using Open-Meteo with Today, 7 Day, and 16 Day modes.
...
> - Directory iPhone Enter/Search keyboard dismissal is fixed in `components/BusinessDirectoryClient.tsx`.
...
1. **Validation pipeline currently non-runnable locally (confirmed)**
   - **File path(s):** `package.json` (implied scripts), `eslint.config.mjs` (import target)
...
1. **Harden map runtime against missing browser geolocation permission APIs (suspected edge-case risk)**
   - **File path:** `components/MapClientMapLibre.tsx`
...
2. **Weather storm classifier may produce false alerts/monitors from broad keyword matching (suspected)**
   - **File path:** `app/api/weather/storm-status/route.ts`
...
3. **Satellite proxy hardening recommendation is now historical (requires redesign before any retry)**
   - **File path:** `app/api/satellite/route.ts`
...
1. **Duplicated weather page logic likely increases maintenance risk**
   - **File path:** `app/weather/page.tsx`, `components/HomeWeatherStrip.tsx`
...
3. **Location persistence may retain outdated fallback labels if future fallback options are introduced (low technical risk now)**
   - **File path:** `components/LocationProvider.tsx`
...
1. **Extract shared location status presenter for weather surfaces**
   - **File path:** `components/HomeWeatherStrip.tsx`, `app/weather/page.tsx`, `components/LocationProvider.tsx`
...
Inspected files: `components/MapClientMapLibre.tsx`, `app/map/MapLoader.tsx`, `app/map/page.tsx`.
...
Inspected files: `components/LocationProvider.tsx`, `components/HomeWeatherStrip.tsx`, `app/weather/page.tsx`.
...
Inspected files: `app/emergency/page.tsx`, `components/SOSButton.tsx`, `components/Nav.tsx`.
...
Inspected files: `app/businesses/page.tsx`, `app/directory/page.tsx`, `components/Nav.tsx`.
...
2. **Potential role-bound guidance overlap**
   - `CLAUDE.md` is role-specific and should remain ignored by Codex except for shared context checks.
...
1. **Possible Drive Mode UX confusion after denied geolocation**
   ...
   - **Files likely involved:** `components/MapClientMapLibre.tsx`, `components/LocationProvider.tsx`
   - **Risk level:** high
...
2. **Storm status misclassification under uncommon NHC wording**
   ...
   - **Files likely involved:** `app/api/weather/storm-status/route.ts`
   - **Risk level:** medium
...
3. **Satellite panel performance degradation on constrained mobile networks**
   ...
   - **Files likely involved:** `app/weather/page.tsx`, `app/api/satellite/route.ts`
   - **Risk level:** medium

## Recommended Next Tasks

1. **Goal:** Restore runnable local validation baseline (lint/build).
   - **Files involved:** `package.json`, lockfile/tooling config, CI scripts.
   - **Risk level:** low
...
2. **Goal:** Add manual regression test script/checklist runs focused on Drive/Plan/recenter/snap-back.
   - **Files involved:** `MAP_REGRESSION_CHECKLIST.md` (and possibly test docs).
   - **Risk level:** low
...
3. **Goal:** Add storm parser fixture-based tests and safer classification heuristics.
   - **Files involved:** `app/api/weather/storm-status/route.ts` + new test fixtures.
   - **Risk level:** medium
...
4. **Goal:** Plan a new scoped satellite proxy hardening design (historical recommendation updated).
   - **Files involved:** `app/api/satellite/route.ts`.
   - **Risk level:** low
...
5. **Goal:** Unify weather location status logic to reduce drift.
   - **Files involved:** `components/HomeWeatherStrip.tsx`, `app/weather/page.tsx`, shared helper.
   - **Risk level:** low
