...
interface MapLibreMap {
  ...
  remove(): void;
  easeTo(options: MapEaseOptions): void;
  getZoom(): number;
  getBearing(): number;
  setStyle(style: RasterStyle): void;
  fitBounds(bounds: MapLibreLngLatBounds, options: FitBoundsOptions): void;
}
...
interface MapLibreMarker {
  setLngLat(lngLat: LngLatTuple): MapLibreMarker;
  addTo(map: MapLibreMap): MapLibreMarker;
  remove(): void;
}

interface MapLibreLngLatBounds {
  extend(lngLat: LngLatTuple): MapLibreLngLatBounds;
}
...
declare global {
  interface Window {
    ...
    __baja411MapLibreLoading?: Promise<MapLibreGlobal>;
  }
}
...
const MAP_CENTER: LngLatTuple = [-110.0, 23.5];
const MAP_DARK_KEY = "baja411-map-dark";
const MAPLIBRE_SCRIPT_ID = "baja411-maplibre-script";
const MAPLIBRE_STYLE_ID = "baja411-maplibre-style";
...
function loadMapLibre() {
  ...
  if (window.__baja411MapLibreLoading) return window.__baja411MapLibreLoading;

  window.__baja411MapLibreLoading = new Promise<MapLibreGlobal>((resolve, reject) => {
    ...
    if (existingScript) {
      existingScript.addEventListener("load", () => {
        ...
        else reject(new Error("MapLibre loaded without a global object."));
      });
  ...
  return window.__baja411MapLibreLoading;
}

function createRasterStyle(dark: boolean): RasterStyle {
  return {
    version: 8,
    sources: {
      "carto-base": {
...
function panelStyle(dark: boolean): CSSProperties {
  return dark
    ? { background: "rgba(5,12,22,0.96)", backdropFilter: "blur(18px)", WebkitBackdropFilter: "blur(18px)", borderColor: "rgba(255,255,255,0.18)", color: "#f8fafc" }
    : { background: "rgba(255,255,255,0.96)", backdropFilter: "blur(18px)", WebkitBackdropFilter: "blur(18px)", borderColor: "rgba(15,23,42,0.14)", color: "#111827" };
}
...
function findCategory(query: string) {
  const q = normalizeText(query);
  if (!q) return null;
  return Object.entries(CATEGORY_SEARCH_TERMS).find(([, terms]) => terms.some((term) => q.includes(normalizeText(term))))?.[0] ?? null;
}
...
function pinMatchesSearch(pin: Pin, query: string) {
  ...
  return [pin.title, pin.description ?? "", label, pin.category, ...categoryTerms].map(normalizeText).join(" ").includes(q);
}
...
export default function MapClientCdnMapLibre() {
  const mapContainerRef = useRef<HTMLDivElement>(null);
  const mapLibreRef = useRef<MapLibreGlobal | null>(null);
  const mapRef = useRef<MapLibreMap | null>(null);
  const markersRef = useRef<MapLibreMarker[]>([]);
  const tempMarkerRef = useRef<MapLibreMarker | null>(null);
  ...
  const latestLocationRef = useRef<LngLatTuple | null>(null);
  ...
  const [visibleCategories, setVisibleCategories] = useState<Set<string>>(() => new Set(CATEGORIES as unknown as string[]));
  const [dark, setDark] = useState(() => (typeof window === "undefined" ? false : localStorage.getItem(MAP_DARK_KEY) === "1"));
  ...
  const visiblePins = useMemo(() => pins.filter((pin) => visibleCategories.has(pin.category) && pinMatchesSearch(pin, search)), [pins, search, visibleCategories]);

  useEffect(() => { followRef.current = following; }, [following]);
  useEffect(() => { modeRef.current = mode; }, [mode]);

  useEffect(() => {
    fetch("/api/auth/session").then((res) => res.json()).then((data) => setSession(data?.user ? data : null)).catch(() => setSession(null));
    refreshPins();
  }, []);
  ...
  useEffect(() => {
    ...
    void loadMapLibre()
      .then((maplibre) => {
        ...
        const handleUserMove = () => {
          ...
          snapBackTimerRef.current = window.setTimeout(() => {
            ...
            setFollowing(true);
            activeMap.easeTo({ center: latest, zoom: Math.max(activeMap.getZoom(), 14), bearing: headingRef.current ?? activeMap.getBearing(), duration: 700, essential: true });
          }, 12000);
        };
        ...
        const handleMapClick = (event: MapMouseEvent) => {
          ...
          tempMarkerRef.current = new maplibre.Marker({ element: createTempMarkerElement(), anchor: "bottom" }).setLngLat([lng, lat]).addTo(map);
      ...
      .catch(() => setMapError("Map engine failed to load. Check connection and refresh."));
  ...
  useEffect(() => {
    const handleOrientation = (event: DeviceOrientationEvent) => {
      ...
      map.easeTo({ bearing: smoothed, duration: 160, essential: true });
    };
  ...
  useEffect(() => {
    ...
    visiblePins.forEach((pin) => {
      ...
      const marker = new maplibre.Marker({ element, anchor: "bottom" }).setLngLat([pin.lng, pin.lat]).addTo(map);
  ...
  useEffect(() => {
    ...
    if (mapRef.current) mapRef.current.getCanvas().style.cursor = placing ? "crosshair" : "";
    if (!showAddModal) {
      tempMarkerRef.current?.remove();
  ...
  function refreshPins() {
    fetch("/api/pins").then((res) => res.json()).then((data) => setPins(Array.isArray(data) ? data : [])).catch(() => setPins([]));
  }

  function startTracking() {
    const maplibre = mapLibreRef.current;
    if (!maplibre || !navigator.geolocation || watchIdRef.current !== null) return;
    setLocating(true);
    setFollowing(true);
    let firstFix = true;
    watchIdRef.current = navigator.geolocation.watchPosition(
      (position) => {
        ...
        const map = mapRef.current;
        if (!map) return;
        const center: LngLatTuple = [longitude, latitude];
        latestLocationRef.current = center;
        if (!locationMarkerRef.current) {
          locationMarkerRef.current = new maplibre.Marker({ element: createLocationMarkerElement(), anchor: "center" }).setLngLat(center).addTo(map);
        } else {
          locationMarkerRef.current.setLngLat(center);
        }
        if (firstFix || followRef.current) {
          map.easeTo({ center, zoom: firstFix ? 14 : map.getZoom(), bearing: headingRef.current ?? map.getBearing(), duration: firstFix ? 800 : 500, essential: true });
          firstFix = false;
        }
        setLocating(false);
        setTracking(true);
  ...
  function recenter() {
    ...
    safeClearTimeout(snapBackTimerRef);
    setFollowing(true);
    map.easeTo({ center: current, zoom: Math.max(map.getZoom(), 14), bearing: modeRef.current === "DRIVE" ? headingRef.current ?? map.getBearing() : map.getBearing(), duration: 650, essential: true });
  }
  ...
  function fitPins(matches: Pin[]) {
    const map = mapRef.current;
    const maplibre = mapLibreRef.current;
    if (!map || !maplibre || matches.length === 0) return;
    if (matches.length === 1) {
      map.easeTo({ center: [matches[0].lng, matches[0].lat], zoom: Math.max(map.getZoom(), 14), duration: 650, essential: true });
      return;
    }
  ...
  return (
    <>
      <div className={`relative h-full w-full ${dark ? "bg-[#060d18]" : "bg-sand"}`}>
        ...
        {mapError && <div className="absolute inset-x-4 top-24 z-[1200] rounded-2xl border border-red-500/30 bg-red-950/90 p-4 text-sm font-bold text-white shadow-2xl">{mapError}</div>}
        ...
        <div className="absolute left-3 right-3 top-3 z-[1000] flex items-start gap-2">
          <div className="min-w-0 flex-1 space-y-2">
            <div className={`inline-flex rounded-full border p-1 shadow-xl ${dark ? "bg-[#050c16]/95 border-white/20" : "bg-white/95 border-black/10"}`}>
              <button type="button" onClick={switchToDrive} className={`rounded-full px-4 py-2 text-xs font-extrabold transition ${mode === "DRIVE" ? activeMode : inactiveMode}`}>Drive</button>
              <button type="button" onClick={switchToPlan} className={`rounded-full px-4 py-2 text-xs font-extrabold transition ${mode === "PLAN" ? activeMode : inactiveMode}`}>Plan</button>
            </div>
            {mode === "PLAN" && (
              <form onSubmit={handleSearchSubmit} className={`flex max-w-xl items-center gap-2 rounded-full border px-4 py-3 shadow-xl ${dark ? "bg-[#050c16]/95 border-white/20" : "bg-white/95 border-black/10"}`}>
                ...
                <input value={search} onChange={(event) => { setSearch(event.target.value); setLastSearchHint(""); }} placeholder="Search towns, fuel, water, beaches" enterKeyHint="search" className={`w-full bg-transparent text-base outline-none ${dark ? "text-white placeholder-slate-400" : "text-slate-950 placeholder-slate-500"}`} />
          ...
          <button type="button" onClick={() => setDark((value) => !value)} className={`${iconButton} ${dark ? "text-white" : "text-slate-950"}`} style={panel} aria-label="Toggle map style">{dark ? "☀️" : "🌙"}</button>
        </div>
        ...
        <div className="absolute bottom-5 right-3 z-[1000] flex flex-col gap-3">
          {mode === "PLAN" && <button type="button" onClick={() => setShowCategoryMenu((value) => !value)} className={`${iconButton} ${dark ? "text-white" : "text-slate-950"}`} style={panel} aria-label="Layers">☰</button>}
          {mode === "PLAN" && <button type="button" onClick={handleAddPinClick} className="h-12 w-12 rounded-full border border-jade/70 bg-jade text-2xl font-light text-white shadow-xl" aria-label="Add pin">+</button>}
          <button type="button" onClick={recenter} className={`${iconButton} ${following ? "text-jade" : dark ? "text-white" : "text-slate-950"}`} style={panel} aria-label="Recenter">◎</button>
        </div>
        ...
        {mode === "PLAN" && showCategoryMenu && (
          <div className="absolute bottom-5 left-3 right-3 z-[1000] rounded-3xl border p-4 shadow-2xl" style={panel}>
            <div className="mb-4 flex items-center justify-between gap-3">
              <div><p className={`text-sm font-extrabold ${textPrimary}`}>Map filters</p><p className={`text-xs ${textSoft}`}>{visiblePins.length} visible pins</p></div>
              <div className="flex gap-2">
                <button type="button" onClick={() => setVisibleCategories(new Set(CATEGORIES as unknown as string[]))} className="rounded-full bg-jade px-3 py-2 text-xs font-extrabold text-white">All</button>
                <button type="button" onClick={() => setVisibleCategories(new Set())} className={`rounded-full border px-3 py-2 text-xs font-extrabold ${dark ? "border-white/20 text-white" : "border-black/10 text-slate-700"}`}>None</button>
                <button type="button" onClick={() => setShowCategoryMenu(false)} className={`rounded-full border px-3 py-2 text-xs font-extrabold ${dark ? "border-white/20 text-white" : "border-black/10 text-slate-700"}`} aria-label="Close filters">✕</button>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
              {(CATEGORIES as unknown as string[]).map((category) => {
                const on = visibleCategories.has(category);
                return <button key={category} type="button" onClick={() => toggleCategory(category)} className={`rounded-2xl border px-3 py-3 text-left text-xs font-extrabold transition ${on ? "bg-jade text-white border-jade" : dark ? "bg-white/8 border-white/15 text-slate-200" : "bg-white border-black/10 text-slate-700"}`}><span className="mr-1.5">{CATEGORY_EMOJI[category]}</span>{CATEGORY_LABELS[category]}</button>;
              })}
            </div>
          </div>
        )}

        {mode === "PLAN" && (search || lastSearchHint) && !showCategoryMenu && <div className={`absolute bottom-5 left-4 z-[1000] rounded-full border px-4 py-3 text-sm font-extrabold shadow-xl ${textPrimary}`} style={panel}>{lastSearchHint || `${visiblePins.length} result${visiblePins.length === 1 ? "" : "s"}`}</div>}
      </div>
          ...
          <div className="pointer-events-auto mx-auto max-w-md rounded-3xl border p-4 shadow-2xl" style={panelStyle(dark)}>
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-4 flex gap-2"><a href={`https://www.google.com/maps/dir/?api=1&destination=${selectedPin.lat},${selectedPin.lng}`} target="_blank" rel="noopener noreferrer" className="rounded-full bg-jade px-4 py-2 text-xs font-bold text-white">Directions</a><button type="button" onClick={() => setSelectedPin(null)} className={`rounded-full border px-4 py-2 text-xs font-bold ${dark ? "border-white/20 text-white" : "border-black/10 text-slate-700"}`}>Close</button></div>
              </div>
              <button type="button" onClick={() => setSelectedPin(null)} className={`text-xl leading-none ${textMuted}`}>×</button>
            </div>
          </div>
        </div>
      )}

      {showAddModal && !pendingLatLng && <div className="fixed bottom-20 left-1/2 z-[200000] flex -translate-x-1/2 items-center gap-3 rounded-2xl border px-5 py-3 shadow-2xl" style={panelStyle(dark)}><span className="text-xl">📌</span><span className={`text-sm font-semibold ${textPrimary}`}>Tap the map</span><button type="button" onClick={() => setShowAddModal(false)} className={`text-xl leading-none ${textMuted}`}>×</button></div>}
      ...
      {showAddModal && pendingLatLng && (
        <div className="fixed inset-0 z-[200000] flex items-end justify-center sm:items-center sm:p-4" style={{ background: "rgba(6,13,24,0.65)" }} onClick={(event) => { if (event.target === event.currentTarget) setShowAddModal(false); }}>
          <div className="max-h-[90vh] w-full overflow-y-auto rounded-t-2xl border p-6 shadow-2xl sm:max-w-md sm:rounded-2xl" style={panelStyle(dark)}>
            <div className="mb-5 flex items-center justify-between"><h2 className={`text-lg font-bold ${textPrimary}`}>Add pin</h2><button type="button" onClick={() => setShowAddModal(false)} className={`text-xl leading-none ${textMuted}`}>×</button></div>
            <form onSubmit={handleSubmitPin} className="space-y-4">
              <input type="text" value={formTitle} onChange={(event) => setFormTitle(event.target.value)} placeholder="Spot name" required className={`w-full rounded-xl border px-4 py-3 outline-none ${dark ? "border-white/15 bg-white/8 text-white placeholder-slate-400" : "border-black/10 bg-white text-slate-950 placeholder-slate-500"}`} />
              <textarea value={formDescription} onChange={(event) => setFormDescription(event.target.value)} placeholder="Notes, access, road condition, warnings" rows={4} className={`w-full rounded-xl border px-4 py-3 outline-none ${dark ? "border-white/15 bg-white/8 text-white placeholder-slate-400" : "border-black/10 bg-white text-slate-950 placeholder-slate-500"}`} />
              <select value={formCategory} onChange={(event) => setFormCategory(event.target.value)} className={`w-full rounded-xl border px-4 py-3 outline-none ${dark ? "border-white/15 bg-[#07111f] text-white" : "border-black/10 bg-white text-slate-950"}`}>{(CATEGORIES as unknown as string[]).map((category) => <option key={category} value={category}>{CATEGORY_LABELS[category]}</option>)}</select>
              <p className={`text-xs ${textSoft}`}>Location: {pendingLatLng[0].toFixed(5)}, {pendingLatLng[1].toFixed(5)}</p>
              {submitError && <p className="text-sm font-semibold text-red-500">{submitError}</p>}
              <div className="flex gap-2"><button type="button" onClick={() => setShowAddModal(false)} className={`flex-1 rounded-full border px-5 py-3 text-sm font-bold ${dark ? "border-white/20 text-white" : "border-black/10 text-slate-700"}`}>Cancel</button><button type="submit" disabled={submitting} className="flex-1 rounded-full bg-jade px-5 py-3 text-sm font-bold text-white disabled:cursor-wait disabled:opacity-60">{submitting ? "Saving…" : "Submit"}</button></div>
            </form>
          </div>
        </div>
      )}
